...
from sawtooth_validator.protobuf.network_pb2 import GossipBatchRequest
...
class Gossip(object):
    ...
    def unregister_identity(self, identity):
        if identity in self._identities:
            ...
            LOGGER.debug("Attempt to unregister identity %s failed: identity "
                         "was not registered")
    ...
    def broadcast_batch_request(self, batch_id, id_type):
        # Need to define node identity to be able to route directly back
        batch_request = GossipBatchRequest(
            id_type=id_type,
            id=batch_id
        )
